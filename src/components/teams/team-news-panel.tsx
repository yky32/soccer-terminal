...
export function TeamNewsPanel({ team, articles }: TeamNewsPanelProps) {
      ...
      >
        <div>
          ...
          <p className="mt-0.5 text-[0.8125rem] text-neutral-500">
            Headlines mentioning {team.name}
          </p>
        </div>
        <Link
          href="/news"
      ...
      {articles.length === 0 ? (
        <div className="px-6 py-14 text-center">
          ...
          <p className="mt-2 text-[0.875rem] text-neutral-600">
            Coverage for {team.name} will appear here as it hits the desk.
          </p>
        </div>
      ) : (
